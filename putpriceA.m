...
function [MP]=putpriceA(S,K,r,T,SIGMA,N,q)
...
%define the matrix of put prices(American)
MP=zeros(N+1);
...
p=(exp((r-q)*t)-d)/(u-d);
%calculate all put prices(American)
for i=N:-1:1
    for j=1:i
      MP(i,j)=max(exp(-r*t)*(MP(i+1,j)*p+MP(i+1,j+1)*(1-p)),K-MS(i,j));
    end
end
end